...
| 阶段 | 里程碑 | 范围 |
| --- | --- | --- |
| M1 (MVP) | 数据接入 + 目录 + 权限 + 基础观测 | 常用连接器、元数据目录、RBAC、REST API |
| M2 | 知识沉淀 + Agent 协同 | RAG 流水线、指标语义层、编排引擎、MCP 工具 |
| M3 | 治理增强 + 反馈闭环 | ABAC、脱敏、血缘可视化、质量评估、迭代闭环 |
| M4 | 规模化 + 开放生态 | 连接器市场开放、Agent 市场、多租户、联邦接入 |
